...
#define SELECT()	P2OUT &= ~CS_PIN	/* CS = L */
#define	DESELECT()	P2OUT |= CS_PIN   /* CS = H */
#define	MMC_SEL		!(P2OUT & CS_PIN)	/* CS status (true:CS == L) */
...
//	printf("\r\nSending cmd: %x\r\n", cmd);
	...
	//crc needs to be sent for CMD0, CMD8 
...
//			for (n=0; n<4;n++){ printf("ocr[%i] = %x\r\n", n, ocr[n]); }
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {			/* The card can work at vdd range of 2.7-3.6V */
				
				//printf("ACMD41 1UL << 30 - %x\r\n", ACMD41);
...
)
{
	...
	if (buff) {		/* Send data bytes */
		...
		if (sa) {	/* Initiate sector write process */
			//printf("initiate sector write..\r\n");
			//printf("CardType=%x CT_BLOCK=%x\n", CardType, CT_BLOCK);
			if (!(CardType & CT_BLOCK)) sa *= 512;	/* Convert to byte address if needed */
			if (send_cmd(CMD24, sa) == 0) {			/* WRITE_SINGLE_BLOCK */
//				printf("WRITE_SINGLE_BLOCK sa=%x\r\n", sa);
				spi_send(0xFF); spi_send(0xFE);		/* Data block header */
				wc = 512;							/* Set byte counter */
				res = RES_OK;
			}
		} else {	/* Finalize sector write process */
//			printf("finalize sector write.. \r\n");
			bc = wc + 2;
			...
			if ((spi_receive() & 0x1F) == 0x05) {	/* Receive data resp and wait for end of write process in timeout of 500ms */
				...
				if (bc) res = RES_OK;
//				printf("recieved data resp and end of write timeout... bc=%i;res=%i\r\n", bc ,res);
			}
			DESELECT();
			spi_receive();
		}
